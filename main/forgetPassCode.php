<?php
    ...
    if(isset($_POST['reset-password-code-submit']))
    {
        $sql = "SELECT one_time_password FROM userinfo WHERE user_email='$email' ";
        ...
        if(empty($verify))
        {
            header("location: forgetPassCode.php?email=$email&code=empty");
        }
        else if($code !== $verify)
        {
            header("location: forgetPassCode.php?email=$email&code=wrong ");
        }
        else if($code == $verify)
        {
            mysqli_query($conn, "UPDATE userinfo SET one_time_password=0 WHERE user_email='$email' ");
            header("location: forgetPassReset.php?email=$email");
        }
            ...
            <div class="row">
                    ...
                    <form action="" method="POST">
                            <div class="form-group">
                                ...
                                <button name="reset-password-code-submit" type="submit" class="btn btn-primary">Next</button>
                    </form>
            </div>
        </div>
        <div class="container">
